...
const columns: Record<KanbanColumn, string> = { ideas: 'Идеи', week: 'На этой неделе', today: 'Сегодня', doing: 'В работе', done: 'Готово' };
const metricLabels: Record<keyof Metrics, string> = { touches: 'новые касания', followUps: 'follow-up', calls: 'звонки', content: 'контент', money: 'деньги', leads: 'лиды', meetings: 'встречи', activeClients: 'клиенты в работе' };
...
function Card({ children, className = '' }: { children: React.ReactNode; className?: string }) { return <section className={`rounded-[28px] border border-white/5 bg-card p-5 shadow-2xl shadow-black/20 ${className}`}>{children}</section>; }
function Button({ children, onClick, type = 'button', className = '', disabled = false }: { children: React.ReactNode; onClick?: () => void; type?: 'button' | 'submit'; className?: string; disabled?: boolean }) { return <button disabled={disabled} type={type} onClick={onClick} className={`rounded-2xl px-4 py-3 font-semibold transition ${disabled ? 'cursor-not-allowed bg-white/5 text-muted' : 'bg-accent text-white hover:scale-[1.02] hover:bg-orange-500'} ${className}`}>{children}</button>; }
function Field(props: React.InputHTMLAttributes<HTMLInputElement>) { return <input {...props} className={`w-full rounded-2xl border border-white/10 bg-black/25 px-4 py-3 text-white placeholder:text-muted ${props.className || ''}`} />; }
function Area(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) { return <textarea {...props} className="min-h-24 w-full rounded-2xl border border-white/10 bg-black/25 px-4 py-3 text-white placeholder:text-muted" />; }
function Progress({ value }: { value: number }) { return <div className="h-3 rounded-full bg-white/10"><div className="h-3 rounded-full bg-good" style={{ width: `${value}%` }} /></div>; }
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      supabase.from('app_state').upsert({ user_id: user.id, data, updated_at: new Date().toISOString() }).then(({ error }) => {
        if (error) notify('Ошибка сохранения в облако');
      });
    }, 500);
    return () => clearTimeout(timer);
  }, [data, user?.id, cloudReady]);
  ...
  const weekDays = useMemo(() => Array.from({ length: 7 }, (_, i) => { const d = new Date(data.sprint.startDate); d.setDate(d.getDate() + i); return d.toISOString().slice(0, 10); }), [data.sprint.startDate]);

  const addTask = (title: string, column: KanbanColumn = 'ideas') => update(d => { if (column === 'doing' && d.tasks.filter(t => t.column === 'doing').length >= 3) { notify('Сначала заверши или убери одну задачу. СДВГ не любит перегруз.'); return d; } const task: Task = { id: uid(), title, description: '', column, done: false, createdAt: todayISO() }; d.tasks.unshift(task); d.sprint.taskIds.push(task.id); return d; }, 'Задача добавлена');
